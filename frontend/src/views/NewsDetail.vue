...
watch(
  ...
  async (newId) => {
    if (newId) {
      // 新闻详情加载完成后，添加到浏览历史
      await newsStore.getNewsDetail(newId);
    }
  },
);
// 组件挂载时获取新闻详情并添加到浏览历史
onMounted(async () => {
  ...
  if (newsStore.newsDetail.id) {
    ...
    if (userStore.getLoginStatus) {
      try {
        const result = await historyStore.addHistoryApi(
  ...
  if (userStore.getLoginStatus && newsStore.newsDetail.id) {
    const result = await favoriteStore.checkFavoriteStatusApi(
      newsStore.newsDetail.id,
    );
    if (result.success && !result.isLocal) {
      ...
      if (
        result.isFavorite &&
        !favoriteStore.isFavorite(newsStore.newsDetail.id)
      ) {
